...
function AdminDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<"analytics" | "verify" | "reports" | "users">("analytics");
  ...
  if (loading || (user && user.role === "admin" && (analyticsLoading || resourcesLoading || reportsLoading || usersLoading))) {
    return (
      <SiteLayout>
    ...
    return (
      <SiteLayout>
        <div className="mx-auto max-w-md text-center py-20 px-4">
          ...
          <Button onClick={() => authModal.show("login")} className="rounded-xl bg-gradient-primary text-white">
            Authenticate
          </Button>
        </div>
      </SiteLayout>
  ...
  const handleDeleteResource = async (id: string) => {
    if (!confirm("Are you absolutely sure you want to delete this resource permanently? This will also remove any related flags.")) return;
    try {
  ...
  const handleDeleteUser = async (id: string) => {
    if (!confirm("Ban and delete this user permanently? This will remove all their shared documents as well.")) return;
    try {
  ...
  const filteredVerifyQueue = resources?.filter((r: any) =>
    !r.verified && (
        ...
        <div className="mx-auto max-w-[1250px] space-y-8">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold text-violet-600 uppercase tracking-widest">AcadVault Control Console</span>
              </div>
              <h1 className="text-3xl font-extrabold tracking-tight mt-1">Administration panel</h1>
            </div>
            <div className="flex gap-2">
              <Button variant="outline" size="icon" onClick={handleRefreshAll} className="rounded-xl border-zinc-200 cursor-pointer h-10 w-10">
          ...
          <div className="flex border-b border-slate-150 overflow-x-auto gap-2 pb-1 text-sm font-semibold tracking-tight scrollbar-none shrink-0">
            {[
              ...
              { id: "verify", label: "Verification Queue", icon: CheckCircle, badge: resources?.filter((r: any) => !r.verified).length },
              { id: "reports", label: "Spam & Flags Dashboard", icon: AlertOctagon, badge: reports?.length },
          ...
          {activeTab === "analytics" && analytics && (
            <div className="space-y-8">
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
                {[
                  { label: "Total Students", val: analytics.totalUsers, icon: Users, color: "bg-blue-500/10 text-blue-650" },
                  { label: "Total uploads", val: analytics.totalResources, icon: FileText, color: "bg-purple-500/10 text-purple-650" },
                  { label: "Verified resources", val: analytics.verifiedResources, icon: CheckCircle, color: "bg-emerald-500/10 text-emerald-650" },
                  { label: "Total downloads", val: analytics.totalDownloads, icon: Download, color: "bg-amber-500/10 text-amber-650" },
                ].map((card, idx) => {
                  const Icon = card.icon;
                  return (
                    <div key={idx} className="bg-white border border-border/60 rounded-3xl p-6 flex items-center justify-between shadow-sm">
                      <div>
                        <p className="text-xs font-bold text-slate-450 uppercase tracking-wider">{card.label}</p>
                        <p className="text-2xl md:text-3xl font-extrabold text-slate-800 mt-2">{formatNum(card.val)}</p>
                      </div>
                      <div className={`h-12 w-12 rounded-2xl ${card.color} flex items-center justify-center shadow-sm`}>
                        ...
                        <div key={branch} className="space-y-1">
                          ...
                          <div className="h-2 w-full bg-slate-100 rounded-full overflow-hidden">
                            <div
                              ...
                              style={{ width: `${Math.min(100, (count / (analytics.totalResources || 1)) * 100)}%` }}
                            />
                          </div>
                        </div>
                      ))
                ...
                <div className="bg-white border border-border/60 rounded-3xl p-6 space-y-4">
                  ...
                  <div className="space-y-3">
                    {analytics.topContributors?.map((u: any, idx: number) => (
                      <div key={u._id} className="flex items-center justify-between p-3 rounded-2xl border border-slate-50 hover:bg-slate-50/50 transition">
                        <div className="flex items-center gap-3">
                          <span className="text-sm font-extrabold text-slate-400 w-4">#{idx + 1}</span>
                          <div>
              ...
              {filteredVerifyQueue.length === 0 ? (
                <div className="text-center py-20 bg-white border border-dashed rounded-3xl p-6">
                  ...
                  <p className="text-sm text-slate-400 max-w-xs mx-auto mt-1">
                    Excellent! All uploaded resources are currently validated or no pending uploads exist.
                  </p>
                </div>
              ) : (
                <div className="grid md:grid-cols-2 gap-6">
                  {filteredVerifyQueue.map((r: any) => {
                    return (
                      <div key={r.id} className="bg-white border border-border/60 rounded-3xl p-5 hover:shadow-md transition flex flex-col justify-between">
                        <div className="space-y-3">
                          <div className="flex justify-between items-start gap-2">
                            ...
                            <span className="text-[10px] text-slate-400 font-semibold">{timeAgo(r.created_at)}</span>
                          </div>
                          <h3 className="font-extrabold text-slate-800 text-sm leading-snug line-clamp-2">{r.title}</h3>
                          <div className="text-xs text-slate-500 font-semibold space-y-1 bg-slate-50 p-2.5 rounded-xl border border-slate-100">
                            <p>Subject: <span className="text-slate-800">{r.subject || "N/A"}</span></p>
                            <p>Branch: <span className="text-slate-800">{r.branch || "N/A"}</span></p>
                            <p>Semester: <span className="text-slate-800">{r.semester || "N/A"}</span></p>
                            <p>Shared by: <span className="text-slate-850">{r.uploader_name || "Unknown"}</span></p>
                          </div>
                        </div>
          ...
          {activeTab === "reports" && (
            <div className="space-y-6">
              {(!reports || reports.length === 0) ? (
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  {reports.map((rep: any) => (
                    <div key={rep.id} className="bg-white border border-red-200/60 rounded-3xl p-5 hover:shadow-md transition flex flex-col justify-between">
                      <div className="space-y-3">
                        <div className="flex justify-between items-start">
                          ...
                          <span className="text-[10px] text-slate-400 font-semibold">{timeAgo(rep.created_at)}</span>
                        </div>

                        <div>
                          <p className="text-slate-400 text-[10px] font-bold uppercase tracking-wider">Reported Resource</p>
                          <h4 className="font-bold text-slate-800 text-sm mt-0.5 truncate">{rep.resource?.title || "Deleted resource"}</h4>
                        </div>

                        {rep.details && (
                          <div className="p-3 bg-red-500/5 rounded-xl border border-red-100 text-xs">
                            <p className="font-bold text-red-750 uppercase text-[9px] tracking-wide mb-0.5">Reporter details</p>
                            <p className="text-slate-700 leading-relaxed font-semibold">"{rep.details}"</p>
                          </div>
                        )}

                        <div className="text-xs text-slate-400 font-medium space-y-0.5 pb-2">
                          <p>Reporter: <span className="font-bold text-slate-800">{rep.reporter?.name}</span> ({rep.reporter?.email})</p>
                          <p>Uploader: <span className="font-bold text-slate-800">{rep.resource?.uploader?.name}</span> ({rep.resource?.uploader?.email})</p>
                        </div>
                      </div>
                        ...
                        filteredUsers.map((u: any) => (
                          <tr key={u._id} className="hover:bg-slate-50/30 transition-colors">
                            <td className="px-6 py-4">
                              <div className="flex items-center gap-3">
                                ...
                                <div className="min-w-0">
                                  <p className="font-extrabold text-slate-800 truncate">{u.full_name}</p>
                                  <p className="text-[10px] text-slate-500 truncate mt-0.5">{u.email}</p>
                                </div>
                              </div>
                            </td>
                            <td className="px-6 py-4 text-slate-650 truncate max-w-xs">{u.college_name}</td>
                            <td className="px-6 py-4 text-slate-800 font-extrabold">{formatNum(u.points)} XP</td>
                            <td className="px-6 py-4">
                              <span className={`inline-flex px-2 py-0.5 rounded-full text-[10px] font-bold ${
                                u.badge.includes("Top") ? "text-amber-700 bg-amber-50" : "text-violet-600 bg-violet-50"
                              }`}>
                                {u.badge}
                              </span>
                            </td>
                            <td className="px-6 py-4 text-center">
                              {u.role === "admin" ? (
                                <span className="text-[10px] text-slate-400 font-bold">Admin Shield</span>
                              ) : (
                                <button
