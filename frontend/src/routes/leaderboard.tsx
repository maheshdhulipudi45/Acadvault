        ...
        <div className="mx-auto max-w-3xl">
          <div className="rounded-[32px] bg-gradient-to-r from-purple-50 via-blue-50 to-green-50 border border-purple-100/50 p-8 md:p-10 mb-8 text-center shadow-[0_15px_40px_rgba(139,92,246,0.02)]">
            ...
            <p className="text-muted-foreground font-semibold">
              Students sharing the most knowledge with the community.
            </p>
          </div>
          ...
          <div className="space-y-3">
            {(data ?? []).map((p, i) => {
              ...
              return (
                <div
                  key={p.id}
                  className={`flex items-center gap-4 bg-white border border-border/60 rounded-2xl p-4 ${i < 3 ? "shadow-card" : ""}`}
                >
                  <div
                    className={`h-12 w-12 rounded-xl flex items-center justify-center font-bold ${i < 3 ? rankColor[i] : "bg-muted text-muted-foreground"}`}
                  >
